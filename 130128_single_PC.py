...
lts = ['-','--','-.',':']
colors = ['k','#5F04B4','#2E9AFE','r','g','c','m','y']
...
for j,df in enumerate(dfs):
	...
	labels=r'$\Delta f = %.2f$' % df
		#if j==3:
			#labels=r'$\Phi_{PEG3500} = %.2f$' % phi_b
		#else:
		#	labels=None
	pl.plot(phis,ps,color = colors[j],linewidth = 3.0,label=labels)#color = colors[j],
	pl.axis([0, 1.0, -0.1, 1.1])

#font = {'size': 22}
#matplotlib.rc('font',**font)
#pl.text(0.86,0.3,r'$\Delta f = 0.0$',color='k')
#pl.text(0.86,0.25,r'$\Delta f = 0.5$', color='r')
#pl.text(0.86,0.20,r'$\Delta f = 1.0$', color='b')
#pl.text(0.86,0.15,r'$\Delta f = 2.5$', color= 'g')
#pl.text(0.86,0.10,r'$\Delta f = 5.0$', color= 'c')
#pl.text(0.86,0.05,r'$\Delta f = 10.0$', color= 'm')
#pl.text(0.86,0.0,r'$\Delta f = 15.0$', color= 'y')

pl.legend(loc='lower right')
#pl.title('Partition Coefficient of PEG400 in a Pore')
pl.xlabel(r'$\phi_{s}$', size= 'x-large')#Number fraction of PEG400')
pl.ylabel(r'$p ( \phi_{s}$ , $\Delta f )$', size= 'x-large')#Partition Coefficient')

pl.savefig('130128_single_PC_poster.eps', dpi=600)
