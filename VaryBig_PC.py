...
Ns = 9
Nb = 79
dfs = [0.0, 4.0, 9.0] #2.5, 5.0, 10.0, 15.0]
...
#	for df in dfs:
	for j,df in enumerate(dfs):
		...
		pl.plot(phibb,ps,'k', marker = markers[j], markerfacecolor= 'w', markevery= 7, markersize= 4, linewidth= 0.5,linestyle = lts[i])#,label=labels)#,color = colors[j],
		pl.plot(-2.0,-2.0,'k', linewidth= 1.0,linestyle = lts[i],label=labels)#,color = colors[j],
		pl.plot(-1.0,-1.0,'k', linestyle= 'none',marker = markers[j], markerfacecolor= 'w', markersize= 6,label=labelss)#,color = colors[j],
...
pl.axis([0.0, 0.93, -0.25, 19.4])

pl.savefig('VaryBig_BW_PC.eps', dpi=600)
