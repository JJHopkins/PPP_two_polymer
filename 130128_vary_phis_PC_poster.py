...
#	for df in dfs:
	for j,df in enumerate(dfs):
		...
		p1 = [phi/phi for phi in phis]#[P(phi_s,phib,df) for phib in phibb]
		...
		pl.plot(phis,ps,color = colors[j],linestyle = lts[i], linewidth= 3.0, markersize= 4, markevery= 7, markerfacecolor= 'w')#label=labels)#color = colors[j],

		pl.plot(phis,p1, 'k', linewidth = 0.5,linestyle = ':')#,label=labels)#,color = colors[j],
		#pl.plot(-2.0,-2.0,'k',linestyle = lts[i], linewidth= 1.0,label=labels)#color = colors[j],
		pl.plot(-1.0,-1.0,color = colors[j],linewidth = 3.0, label=labelss)#color = colors[j],
...
pl.axis([0, 0.85, -0.1, 2.1])

pl.savefig('130128_vary_phis_PC_poster.eps', dpi=600)
