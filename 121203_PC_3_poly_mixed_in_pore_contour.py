...
for h,phi_m in enumerate(phi_ms):
	...
	for i,phi_b in enumerate(phi_bs):
		...
		#	pmms = [pms*phi_ms]
			...
			Phis,Phis_in = np.meshgrid(phis, phis_in)
			Pms = np.Pm(Phis,phi_m,phi_b,Phis_in,df)
		...
		#		labelss=None
			...
			pl.contour(phis,phis_in,pms)#,[0.5, 1.0, 1.2,1.5], colors='k', linestyle = 'solid')#marker= markers[h],color=colors[j],linestyle = lts[i], linewidth= 0.7, markersize= 4, markevery= 7, markerfacecolor= 'w')
...
#pl.minorticks_on()
pl.savefig('121203_PC_3_poly_Pm_in_with_true_phis_at_2300hr_contour.eps', dpi=600)
