...
#			pl.semilogy(0.005,0.005,'k',linestyle = '--',label=r'$\phi_{b} = 0.10$')
			...
			#pl.figure(4)
			#pl.semilogy(dfs_cont,pms_df_cont,marker=markerss[h],color=colors[k],linestyle=lts[i],linewidth=0.7,markersize=4,markevery=7)
			#pl.title(r'Fig.4 Zoom Log Fig.3 Pm for varied $\Delta f$,fixed $\Phi_{s}^{out}$,$\Phi_{m}^{in,\ out}$,$\Phi_{b}^{out\ only}$', size = 'x-large')
			#pl.xlabel(r'$\Delta f$', size = 'x-large')
			#pl.ylabel(r'Pm($\Phi_{s}^{i,o}$,$\Phi_{m}^{i,o}$,$\Phi_{b}^{o}$,$\Delta f$)', size = 'x-large')
...
#			pl.legend(loc='upper right')
			...
			#pl.figure(6)
			#pl.semilogy(dfs_cont,phim_in_df_cont,marker=markerss[h],color=colors[k],linestyle =lts[i],linewidth=0.7,markersize=4,markevery=7)
